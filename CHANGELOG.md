...
## 2026-09-18
- feat(eventsink): real Redis Streams durable queue primitives (RedisStreamSink + RedisStreamConsumer), the "switch to a durable queue for fanout to other stores like mysql" decision for the Kubernetes migration's zero-downtime cutover plan (S498) -- founder real-time: "part of the kubes plan was to switch to a queue so we can have a durable queue to work from for fanout to other stores like mysql." RedisStreamSink implements the existing EventSink interface (XADD, drops into FanoutSink alongside FileSink/S3Sink); RedisStreamConsumer wraps a real Redis consumer group (XREADGROUP/XAck) -- one independent, durably-tracked read position per downstream consumer (processor, a future GCS archiver, a future MySQL sink). This is the real mechanism the zero-downtime cutover plan rests on: a new k8s pod joins the SAME consumer group as an additional consumer before the old systemd process stops, and Redis's own per-group delivery (tracked in the group's Pending Entries List until Ack) guarantees every entry is claimed by exactly one live consumer with zero gap and zero duplication in steady state. Verified against a real, in-process, real-protocol Redis implementation (miniredis/v2, no live Redis/Memorystore needed to prove the stream mechanics) -- 5 new tests, including direct proof of the PEL-based single-delivery property and that a new consumer joining an existing group picks up exactly the remaining, un-acked work. Queue backend decided: Redis Streams on GCP Memorystore for Redis (Standard tier, HA), not a self-hosted Redis on a bare PVC -- removes both the failover-logic burden and the "do we need a volume for this" question, since it's a managed service. Full architecture writeup, including the real split between producer-side cutover safety (idempotent dedup via stable Event.ID, no queue primitive needed) and consumer-side cutover safety (the consumer-group mechanism above), in docs/northstar/KUBERNETES_MIGRATION.md's own new section. Real, honest, not yet done: no live Memorystore instance provisioned, no cmd/ process actually wired to publish/consume via these types yet, and GKE cluster health unconfirmed this pass (last audit found zero working compute nodes for 32+ hours, no live gcloud/kubectl credentials in this session to re-check). go build/go vet/go test ./... all clean.
...
## 2026-09-02
- S233-06: BusinessWire scraping work PAUSED, per founder direction ("disable it for now until we can figure out a better idea"). Not a code problem: the entire webdriver+HTTP-client chain built this session works correctly end to end (verified live against a real Chrome+chromedriver and example.com). The real, final blocker is this box's own outbound IP (`AS63949 Akamai Connected Cloud`, a Linode datacenter range) being broadly flagged by anti-bot systems -- confirmed by a real Chrome session getting `ERR_HTTP2_PROTOCOL_ERROR` on businesswire.com and a live reCAPTCHA challenge on google.com. No code change fixes an IP-reputation problem; unblocking this needs a real residential/mobile proxy or a non-datacenter IP, a founder-level infrastructure decision. `docs/CONTENT_TYPE_TAXONOMY.md` updated with an explicit "PAUSED, do not retry without a proxy plan" marker so this doesn't get silently re-attempted. `prwatch.RunnerConfig.SourceName`/`SourceProvider`/poll-jitter all stay in place, dormant (safe defaults, zero behavior change for live PRNewswire polling) until that decision is made. (sess-20260830-1207-cc0ba7da)

- S233-02: real, optional poll-timing jitter/ranging for `cmd/prwatch`. Founder real-time, ahead of an eventual BusinessWire runner: "can we still add some basic jitter in the timing of the runner for the businesswire one?" -> "also have it crawl and then rest between 15seconds and 1 minutes +-10 and then crawl again." New flags: `-poll-interval-min`/`-poll-interval-max` (each round's base wait becomes a fresh uniform random duration in that range instead of the fixed `-poll-interval`) and `-poll-jitter` (an additional +/- randomization on top of whichever base is used). All three default to their old exact values/off, so an unmodified `prwatch` invocation (PRNewswire's real production polling) is byte-for-byte unchanged. Recommended real values for a future BusinessWire runner, named directly in the flag help text: `-poll-interval-min=15s -poll-interval-max=1m -poll-jitter=10s`. New `main_test.go` (previously no test file existed for this cmd): bounds-checked jitter, never-negative-wait, unset-range-falls-back-to-fixed, and the exact recommended BusinessWire range all asserted. Scope note: this is basic pacing variance only, not active evasion -- an occasional-decoy-click/"wander the site" idea was raised in the same conversation and then explicitly paused by the founder before anything was built; not present here. `go build ./...` and `go test ./...` both clean. (sess-20260830-1207-cc0ba7da)

- S233: real press-release provider/subtype ("prtype") field, end to end, plus a written answer to "do we have a content-type hierarchy?" (`docs/CONTENT_TYPE_TAXONOMY.md`, golden-indexed). Founder real-time: "add businesswire prs to our data sources same as prnewswire but businesswire... tagged as... type pressreleases... individual businesswire and prnewswire as options for prtype... do we have a good sense of how we are storing the data in terms of content type?" Found and fixed a real, existing bug along the way: `prwatch/runner.go` hardcoded `Source: "prnewswire"` in two places regardless of what the `Client` was actually configured to scrape -- replaced with `RunnerConfig.SourceName` (defaults "prnewswire", zero behavior change for existing callers). Threaded the real source name end to end: `PressReleaseDiscovered.Source` -> new `BodyFetchedEvent.Source` (mirrored through `crawler.go`) -> new `SourceDocument.SourceProvider` (`pkg/intelligence`) -> new `DocSummary.SourceProvider` (`internal/newssite/docindex`) -> new `?provider=` query filter on `GET /v1/press-releases/{ticker}` (`internal/apiserver`, OpenAPI doc updated). Reused the existing `SourceType` field as the real content-type dimension rather than inventing a parallel `content_type`/`prtype` pair -- `SourceProvider` is the one new subtype field, empty for anything that isn't a press release. Honest, checked-not-assumed limitation: no working BusinessWire scraper shipped this pass -- `businesswire.com`'s news pages return 403 to this session's own fetch tooling and their real RSS needs a subscriber-specific token, so writing scrape regexes against unseen markup would be guessing, not shipping; flagged explicitly in the new doc rather than silently faked. Verified: `go build ./...` and `go test ./prwatch/... ./internal/apiserver/... ./internal/newssite/docindex/... ./pkg/intelligence/... ./cmd/pr-indexer/...` all pass. EDIS's `edis_press_releases` shortcode/template updated in the same pass to prove the type/subtype plumbing on the WordPress-as-consumer side (see EDIS's own CHANGELOG). (sess-20260830-1207-cc0ba7da)

## 2026-08-30
- feat: real Helm chart for dashboard (charts/dashboard/), superseding the raw k8s/dashboard.yaml manifest. Founder real-time: "write kubernetes primitives into the stdlib and write helm support into parena both in the stdlib and also into DUNG and project BURROW". Generated from PARENA's own new stdlib/k8s + stdlib/helm primitives (Chart.yaml, values.yaml with real Helm-standard replicaCount/image.repository/image.tag shape, templates/deployment.yaml with real {{ .Values.X }} substitution, templates/service.yaml, templates/pvc.yaml) -- the exact same real dashboard values k8s/dashboard.yaml already used. YAML-syntax-validated. Real, honest, unverified: no `helm` binary in the authoring session to run `helm lint`/`helm template` against it. k8s/dashboard.yaml kept, not deleted -- real documentation of the pre-Helm shape. KUBERNETES_MIGRATION.md updated to note the supersession. (sess-20260830-0908-5ede695f)
...
- feat(entity-graph): Phase 3 (1/2) of the replay-fragility fix -- checkpoint freshness heartbeat + ops runbook. entity-graph's two SQLite index checkpoints (`filings-index.db`, `accuracy-index.db`) previously only updated `meta.snapshot_at`-equivalent state when a batch happened to contain matching records (filing_discovered / newly-resolved accuracy verdicts) -- most batches don't, so there was no way to tell "checkpoint healthy but idle" from "checkpoint write path silently wedged." New `touchFilingIndexSnapshot`/`touchAccuracyIndexSnapshot` write `meta.snapshot_at` unconditionally once per `runBatch` tick (right after a successful `store.ReadFrom`), mirroring how `CheckPollerHealth`'s log-mtime heartbeat already treats "checked, nothing new" as proof of life for headless pollers. Also documented the "checkpoints are a disposable cache, deleting any one is always a safe operator move" invariant in `docs/ops-runbook.md`, with the exact delete-and-restart recovery steps. `go test ./...` green (2 new tests). Live-verified: stopped the real `fatbaby-entity-graph.service`, ran the new binary in `-one-shot` mode against production data (983 accuracy records, matching prior counts), confirmed both checkpoint files' `meta.snapshot_at` populated for the first time, rebuilt and restarted the service, confirmed the timestamp continued advancing under its own systemd-supervised poll loop (not just the one-shot run). Freshness-check consumer (`EMILY/emily-agent`'s watchdog) lands as a separate EMILY-repo commit per the multi-repo dependency-order convention.
...
- feat(newssite): earnings dates now shown on ticker pages (next upcoming + up to 4 most recent past, with status/timing) via the already-loaded earningscal.Store, previously only used on /section/earnings. Found and fixed a real data sourcing bug along the way: 61 of 363 stored earnings-calendar records had FiscalYear=0 (scanSecFilings didn't handle ExtractPeriod finding a quarter label but no explicit year in filing text). Fixed the extraction fallback (derive year from filing date), added a store-level filter for the now-orphaned bad-ID legacy records (append-only store can't delete, corrected records land under new IDs), and re-derived+corrected all 61 live records. New tests for both. PRRJECT_FATBABY 7fc2b54 + 95764ee.
- fix(processor): persist cursor across restarts instead of replaying full history from seq 1 on every start — fourth instance of the replay-fragility bug (after signalapi/newssite/entity-graph), found live while investigating an active OOM crash-loop (processor pinned at its 256M cap + 282M swap). loadSeenIdentities now returns latest_seq via Scan so Run resumes the main loop from there instead of hardcoding lastSeq=1; main loop also moved onto Scan. Cold-start catch-up: ~1hr -> ~7s, memory flat ~7-10MB instead of climbing toward the cap. Also rebuilt+committed the stale tracked bin/newssite and bin/signalapi deploy binaries, which were still pre-Phase-0 and were the actual trigger behind newssite's own live OOM crash-loop (restart counter climbing every ~3-7min) that took secwatch/processor down as system-wide memory-pressure collateral. PRRJECT_FATBABY ff92e2c, ef6c5f8.
- feat(eventstore): Phase 0 of the replay-fragility fix — streaming Scan(ctx, fromSeq, fn) API on eventstore.FileStore replaces the O(n`)^2`) ReadFrom read path (was re-reading and re-JSON-decoding the entire journal file per 512-record page; the 354MB 2026-07-16 journal took ~39 full passes). Migrated signalindex.Build/Tail, docindex.Build/Tail, and entity-graph's buildFilingIndexes onto Scan; ReadFrom untouched for targeted single-record fetches. Added -replay-from-seq emergency degraded-mode flag to signalapi/newssite per the northstar's §5. Also fixed Tail()'s first poll waiting a full pollInterval (default 30s) before signaling ready, in both signalindex and docindex — was masked by the O(n^2) bug, became the dominant cost once that was fixed. Live-verified against the real var/secwatch store (630MB/116K records): signalapi cold rebuild was previously unable to complete (killed twice at 628MB/540MB RSS with swap near-exhausted); now completes in 30.1s wall-clock at 60.6MB peak RSS (northstar acceptance target: <60s, <300MB). 5 new eventstore tests for Scan (ReadFrom-equivalence, tail-resume-without-reprocessing, no-op-on-no-new-events, truncated-trailing-line tolerance, fn-error-stops-scan). go test ./... green. Phases 1-3 (SQLite checkpoints, entity-graph incremental tables) remain per docs/northstar/replay-fragility.md.
- docs: `docs/northstar/replay-fragility.md` — NORTHSTAR for the full-history-replay-on-restart fragility that took `signalapi` down twice in one night (stall at ~109K records, RSS 628MB/540MB against a full swap partition) and drove `newssite`'s OOM crash-loop. Root cause found in code, not guessed: `eventstore.FileStore.ReadFrom` re-reads and re-parses the **entire** journal file for every 512-record page — against the 354MB 2026-07-16 journal that is ~39 full passes (~13.8GB of redundant JSON decoding, ~400MB+ transient allocation per pass, store mutex held throughout), and the 30s `Tail` pollers hit the same path against the never-cached current-day file on every poll. Decision made (founder asked for a decision, not a menu): per-process SQLite checkpoints (snapshot-plus-tail — `modernc.org/sqlite` already in go.mod, same pattern as IDUNA blog/mailinglist, and the exact `TODO(scale)` seam written into `internal/signalindex/index.go`) on top of a Phase-0 streaming `Scan` API that deletes the O(n²) read path. Mongo-as-cache evaluated and rejected (new 256MB+ always-on process on a 3.8GB box — SECTION 152's lesson); bounded replay window rejected as primary (data-loss policy in a performance-fix costume) but kept as a `-replay-from-seq` emergency flag. Also documents entity-graph's three per-batch reload sins (full-store `buildFilingIndexes` scan every 30s batch; `accuracy.ndjson` at 482,696 lines/136MB of recomputed duplicates reloaded every batch; whole graph rebuilt from NDJSON every batch) with a phased fix. Design only — no code yet; 5 phases, each independently shippable, each gated on a live kill-test.
- docs: `docs/northstar/tina-engine.md` — NORTHSTAR for TINA (Trading Idea, Not Advice), the compliance-forward layer that turns FatBaby's already-detected signals (insider activity, dividends, buybacks, guidance, EPS, governance/succession) into structured, sourced, explicitly-not-advice trading ideas. Positioned as the substrate underneath `jon-agent`'s existing chat persona, not a replacement for it — Jon is voice, TINA is the disclaimer-forward structured data underneath. Inherits entity-graph's own disclosed-precision honesty (11.9% overall, some signal types at 0%) as the model for TINA's mandatory `confidence_note` field. Design only, phased build plan, explicit open questions (continuous-generation risk given S153-14, legal review, GTM tier placement all flagged rather than assumed).

## 2026-07-18 (4)
- fix(newssite): `serveDoc` was doing a full linear scan from sequence 1 through the entire event-store history (`ReadByIdentity`) on **every single article page view**, not just at startup — found while debugging user-reported broken deep links (a request took 60s, hit nginx's `proxy_read_timeout`, returned 504). `docindex.Index` already builds an in-memory `byIdentity` map with exactly the `Sequence` needed for a targeted lookup, but `serveDoc` wasn't using it. Two real bugs fixed: (1) `docindex.Ingest` never actually set `Sequence` on the `DocSummary` it builds, despite the field existing and being documented for this purpose — a test written against the new `Index.ByIdentity` accessor caught this immediately (`got Sequence 0`); (2) `serveDoc` now tries `docIdx.ByIdentity` + a new `ReadAtSequence` (targeted single-record fetch, touches at most one day-partition file) first, falling back to the old full-scan `ReadByIdentity` only when the index hasn't indexed that identity yet (e.g. mid-rebuild). Live-verified real improvement: an already-indexed doc that previously would have taken 60s+ now resolves in 1.85s. **Honest caveat, not fixed here**: `newssite` is still actively OOM-crash-looping (SECTION 152/153's S153-14, queued as tomorrow's top-priority Fable dispatch) — a doc not yet indexed when the process crashes again still falls back to the slow path, and the underlying replay-on-restart fragility is the real fix, not this handler-level optimization alone. 5 new tests (2 `docindex.ByIdentity`, 1 handler fast-path integration test, existing suite unaffected).
...
- fix(ops/S152-03): user-level systemd supervision (`ops/systemd/fatbaby-{secwatch,processor,prwatch,prwatch-body,eps-reconciler}.service`) for the five headless pollers that had no restart-on-crash and no boot survival — root cause of secwatch's ~10h and eps-reconciler's ~22h silent outages. Replaces manually-launched `go run` processes with static binaries (`bin/`) under systemd, matching the proven `iduna.service` pattern (no sudo, `WantedBy=default.target`). Live-verified: SIGKILL'd `prwatch` to simulate an OOM-kill, confirmed systemd auto-restarted it within 10s. `processor`'s `ANTHROPIC_API_KEY` now comes from `~/.config/fatbaby/env` (outside the repo), not the previously-mistracked `ops/env.production`. See EMILY BACKLOG SECTION 152.
...
- feat(S141-03): `internal/entitygraph/norngate` — migrates entity-graph's signal accuracy/correlation system (the "recon matcher" leg) to the NORN kernel, NORN's second real instantiation. Unlike S141-02, real historical grading ground truth exists in force: `var/entity-graph/accuracy.ndjson` has ~345K records, ~32.7K resolved (confirmed/refuted). `AccuracyOracle` grades a frozen, content-addressed snapshot of the resolved records (live file excluded — PRIME-101 §5 requires frozen evals to hold still, and 313K pending records are still resolving) via the existing `entitygraph.BuildAccuracyReports`. `cmd/norn-entitygraph-migrate` bootstrap-promoted the current `config/entity-graph-rules.json` as the first `entity_graph_rules` artifact — run for real against the full 345K-record production file in ~3.8s, `var/norn/entity_graph_rules.ndjson`. Real finding surfaced as a side effect, not fixed here: overall precision is 11.9% across 32,664 resolved predictions, with `abstention_outlier`, `cfo_departure`, and `family_control` sitting at exactly 0% — flagged in BACKLOG.md as a follow-up, not investigated in this migration. Disclosed gap: Grade measures the rules' real production track record, not a genuine candidate-vs-incumbent backtest (that needs a re-simulation capability — replay raw filings under a candidate ruleset — that doesn't exist yet). 6 new tests. GEE (GameEvolutionEngine) leg of S141-03 investigated and found unbuildable as a migration target: no such system exists anywhere in the codebase (verified — only spec-table references), only a loosely related, differently-shaped bot-genome mutation system in SHANKPIT with no external oracle and no gate; disclosed as a gap in BACKLOG.md rather than building a new game-design engine under a "migration" heading.
- refactor: extracted the bootstrap-or-gate branching logic (grade → record → promote-if-no-incumbent-else-gate) into `norn.GradeAndPromote` (NORN repo) after writing it by hand for S141-02 and being about to write it again for S141-03 — exactly the re-derivation PRIME-101 exists to prevent. `internal/eps/norngate/migrate.go` refactored to call the shared helper; both migrations now share one code path.
...
- Founder-requested audit: 7 processes found running unsupervised or not running at all (eps-processor via go run, entity-graph via go run, dividend/buyback/guidance/nt-watcher and earnings-calendar not running at all). All now under real systemd units, verified live with zero restarts. earnings-calendar's month-long silent outage (dead since 2026-06-17) is the real root cause of the stale ticker-page earnings dates reported earlier. form4-watcher and schd13-watcher found to hang indefinitely -- real bug, tracked separately, not started under supervision
- Rewrote README to reflect real scale (36 cmd/ binaries, 40 internal/ packages, 630MB+ event store, 50 tickers) and document the reusable engineering patterns (eventstore Scan, Build/Tail indexes, indexcheckpoint, httpretry, marketcal gating, tickerlink editorial standard, systemd supervision, auth-on-writes)
