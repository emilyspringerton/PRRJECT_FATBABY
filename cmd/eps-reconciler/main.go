...
func main() {
	...
	logger.Printf("starting poll_interval=%s store=%s eps_dir=%s", *pollInterval, *storeRoot, *epsDir)

	for {
		reconcile(ctx, store, *epsDir, logger)
...
func reconcile(ctx context.Context, store eventstore.EventStore, epsDir string, logger *log.Logger) {
