...
func (s *server) handleGovernanceSignals(_ http.ResponseWriter, r *http.Request) (int, any) {
	...
	query := "SELECT id, ticker, event_type, filing_id, entity_name, signal_score, headline, filing_date, COALESCE(source_published_at,''), ingested_at, eventstore_seq FROM governance_signals " + where + " ORDER BY COALESCE(source_published_at, filing_date) DESC LIMIT ?"
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.ID, &row.Ticker, &row.EventType, &row.FilingID,
			...
			&row.SourcePublishedAt, &ingestedAt, &row.EventstoreSeq); err != nil {
			continue
		}
...
// handleSignalsByTickerMySQL serves GET /v1/signals/{ticker} from MySQL governance_signals.
// Ordered by source_published_at DESC (original document date), falling back to filing_date.
// Query params:
//
//	limit        — max results (default 15, capped at MaxLimit)
//	signal_type  — filter by event_type
//	from         — YYYY-MM-DD lower bound on source_published_at
func (s *server) handleSignalsByTickerMySQL(ticker string, r *http.Request) (int, any) {
	...
	where := "WHERE ticker = ?"
	args := []any{ticker}
	if sigType := strings.TrimSpace(q.Get("signal_type")); sigType != "" {
		where += " AND event_type = ?"
		args = append(args, sigType)
	}
	if from := strings.TrimSpace(q.Get("from")); from != "" {
		where += " AND COALESCE(source_published_at, filing_date) >= ?"
		args = append(args, from)
	}
	args = append(args, limit)

	rows, err := s.cfg.MySQL.QueryContext(r.Context(),
		"SELECT id, ticker, event_type, filing_id, entity_name, signal_score, headline, filing_date, COALESCE(source_published_at,''), ingested_at, eventstore_seq FROM governance_signals "+
			where+" ORDER BY COALESCE(source_published_at, filing_date) DESC LIMIT ?",
		args...,
	)
	if err != nil {
		s.cfg.Logger.Printf("signals-by-ticker-mysql ticker=%s: %v", ticker, err)
		return http.StatusInternalServerError, map[string]string{"error": "query failed"}
	}
	defer rows.Close()
